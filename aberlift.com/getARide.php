...
{
	document.getElementById("offerWindow").style.display = "block";
	document.getElementById("offerContainer").innerHTML = "<svg class='center spinner' style='margin-left:384px;margin-top:193px' width='32px' height='32px' viewBox='0 0 66 66' xmlns='http://www.w3.org/2000/svg'><circle class='path' fill='none' stroke-width='6' stroke-linecap='round' cx='33' cy='33' r='30'></circle></svg>";

	xmlhttp = new XMLHttpRequest();
	xmlhttp.onreadystatechange = function () {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			var Reply = xmlhttp.responseText;
			document.getElementById("offerContainer").innerHTML = Reply;
		}
	}
	xmlhttp.open("POST","getOfferDetails.php",true);
	xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	xmlhttp.send("offerID=" + offerID);

	xmlhttp.onerror = function() {
		setTimeout(function(){ document.getElementById("offerContainer").innerHTML = "<div style='margin-left:auto;margin-right:auto;max-width:200px;padding-top:50px;'><img src='Resources/svg/no_internet.svg' style='min-width:200px;min-height:200px;margin-left:auto;margin-right:auto;'></div><div style='margin-left:100px;margin-right:100px;margin-top:50px;color:rgba(0,0,0,0.30);-webkit-font-smoothing: antialiased;line-height:18px;font-size:14px;'>Looks like your connection to the internet has been lost. There might be an issue with your Network settings. <span style='cursor:pointer' onclick='viewOffer(\"" + offerID +  "\")' class='accent'>Retry?</span></div><div onclick='cancelOffer()'' class='header-button' style='display:block;width:60px;margin-left:auto;margin-right:auto;margin-top:40px;''>close</div>"; }, 500);		
    };

	setTimeout(function(){ document.getElementById("offerWindow").style.opacity = "1"; }, 0);
	setTimeout(function(){ document.getElementById("offerContainer").style.boxShadow = "0 19px 38px rgba(0,0,0,0.30), 0 15px 12px rgba(0,0,0,0.22)"; }, 100);
}
...
function acceptOffer(offerID)
{
	...
	xmlhttp.open("POST","acceptOffer.php",true);
	xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	xmlhttp.send("offerID=" + offerID);
...
{
	document.getElementById("offerContainer").style.boxShadow = "0px 0px 0px rgb(0,0,0)";
	document.getElementById("offerWindow").style.opacity = "0";
	setTimeout(function(){ 	document.getElementById("offerContainer").innerHTML = ""; document.getElementById("offerWindow").style.display = "none"; }, 500);
...
<body onload="loadDate();fixBoarding();fixDestination();">
	...
	<div id="offerWindow" class="topWindow">
		<div id="offerContainer" class="confirmationContainer">
		</div>
	</div>
	<div id="container-frame" class="everything-container clearfix">
		<div class="headline">Available Offers</div>
		<div class="sub-line">Before you post the request for a lift, please have a look at the offers below. You can directly accept an offer instead of posting your request.</div>
		<!--********** FILTERS **********-->
		...
		<div class="right-container">
			<div class="notif-top">
				<div class="text-black">If you do not find a suitable lift offer below, you can
				<?php
					...
					echo '<a href="getRide.php?From=' . $boardingPoint . '&To=' . $destination . '&Date=' . $boardingDate . '&Time=' . $boardingTime . '">';
				?>
				<span class="bold accent">request a new one.</span></a>
				</div>
			</div>
			<div id="availableOffers">
				<?php include 'showOffers.php'; ?>
			</div>
		</div>
	</div>
	...
	<script>
		...
		{	
			...
			document.getElementById("availableOffers").innerHTML =  "<svg class='center spinner' style='margin-left:384px;margin-top:193px' width='32px' height='32px' viewBox='0 0 66 66' xmlns='http://www.w3.org/2000/svg'><circle class='path' fill='none' stroke-width='6' stroke-linecap='round' cx='33' cy='33' r='30'></circle></svg>";
			
			xmlhttp = new XMLHttpRequest();
			xmlhttp.onreadystatechange = function () {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					document.getElementById("availableOffers").innerHTML = xmlhttp.responseText;
				}
			}
			xmlhttp.open("GET","showOffers.php?From=" + boardingCode + "&To=" + destination + "&Date=" + date + "&Time=" + time, true);
			xmlhttp.send();
		}
	</script>
