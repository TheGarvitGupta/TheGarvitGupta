...
function offerRide()
{
	...
	document.getElementById('capacity-error').style.opacity = "0"; 

	var boardingPoint = document.getElementById('boardingPoint').value;
	var destination = document.getElementById('destination').value;
	var boardingDate = document.getElementById('hbdsdf').value;
	var boardingTime = document.getElementById('time').value;
	var ridecapacity = document.getElementById('capacity').value;
	...
	if (ridecapacity == "")
	{
		document.getElementById('capacity-error').style.opacity = "1";
		flag++;
	}
	...
	if(flag == 0)
	{
			...
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				if (Reply == "success")
				{
					...
					setTimeout(function(){
						...
						document.getElementById("button-container").innerHTML='<div onclick="offerRide()" class="center header-button" style="display:inline-block;margin-top:50px">Publish Offer</div>';
					}, 250);
				}
				...
				{
					//unknown error
					document.getElementById("message-window").innerHTML = '<img src="Resources/svg/error.svg" style="min-width:200px;min-height:200px;margin-top:25px;margin-left:300px;margin-right:300px;"><div style="margin-left:100px;margin-right:100px;margin-top:10px;color:rgba(0,0,0,0.87);-webkit-font-smoothing:antialiased;line-height:18px;font-size:14px"><div style="text-align:center;font-size:18px;line-height:32px"class="accent">Well, that\'s embarrasing</div><br>There was an unknown error while trying to post your lift offer. Please try posting the offer again. If nothing works, sign out and sign in again.</div><div onclick="closeMessage()" class="header-button" style="display:block;width:60px;margin-left:auto;margin-right:auto;margin-top:40px;">close</div>';
					document.getElementById("messageWindow-full").style.display = "block";
		...
		xmlhttp.open("POST","publishRide.php",true);
		xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		xmlhttp.send("from=" + boardingPoint + "&to=" + destination + "&date=" + boardingDate + "&time=" + boardingTime + "&seats=" + ridecapacity);
	}
}
...
<body onload="loadDate();fixDestination();fixBoarding();">
	...
	<div class="everything-container clearfix">
		<div class="headline">Create an offer</div>
		<div class="create-ride-form">
			<div class="center sub-sign-content">
				
				<select onblur="fixDestination()" id="boardingPoint" class="select verticalFormSpecs object" style="margin-top:0;">
					<?php
						/* If no GET header */
				...
				<select id="capacity" class="select verticalFormSpecs object">
					<option value="" disabled="" selected="">Available Seats</option>
				...
				<div id="capacity-error" class="error">Please enter the available seats</div>

				<div id="button-container" style="text-align:center;">
					<div onclick="offerRide()" class="center header-button" style="display:inline-block;margin-top:50px">Publish Offer</div>
				</div>
			</div>
		</div>
		<div class="create-ride-card">
			...
			<div class="ride-line-container">
				<div style="float:left;height:32px;width:32px;background:url('Resources/svg/seats.svg');background-repeat:no-repeat;background-size:32px;background-position:0px 0px;"></div>
				<div class="confirmationDetailsMark">
					<div id="gCapacity" class="Head-graphic-location"></div>
					<div class="Sub-graphic-location">Available Seats</div>
				</div>
			</div>
		</div>
	</div>
	...
	<script>
		...
		$(document).on('change','#capacity',function(){ refreshGraph(); });
		...
		{	
			...
			document.getElementById("gCapacity").innerHTML = $( "#capacity option:selected" ).text();
		}
