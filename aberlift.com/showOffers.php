...
<?php
	...
	$Query = "SELECT * FROM AberLift_OFFERS WHERE status='active' AND from_code LIKE '$From' AND to_code LIKE '$To' AND ride_date >=$Date AND ride_time >=$Time ORDER BY ride_date, ride_time, offer_timestamp ";
	$result = mysqli_query($con,$Query);
	if (mysqli_num_rows($result) > 0)
	{
		while($row = mysqli_fetch_array($result))
		{	
			echo'<div onclick="viewOffer(\'' . $row['offer_id'] . '\')" class="ride-container">';
			...
			/* echo offerer name using offerer_id */
	...
	else
	{
		...
		if (($From == "%") && ($To == "%"))
		{
			echo "<div class='no-ride-found-message'>Looks like nobody has posted an offer for a lift for the selected date and time. You can try searching for a different date and time or publish your own request for the lift. We'll let you know as soon as someone offers you a lift.</div>";
		}
		else if ($From == "%")
		{
			echo "<div class='no-ride-found-message'>Looks like nobody has posted an offer for a lift to " . $ST2Name . " for the selected date and time. You can try searching for a different date and time or publish your own request for the lift. We'll let you know as soon as someone offers you a lift.</div>";
		}
		else if ($To == "%")
		{
			echo "<div class='no-ride-found-message'>Looks like nobody has posted an offer for a lift from " . $ST1Name . " for the selected date and time. You can try searching for a different date and time or publish your own request for the lift. We'll let you know as soon as someone offers you a lift.</div>";
		}
		else
		{
			echo "<div class='no-ride-found-message'>Looks like nobody has posted an offer for a lift from " . $ST1Name . " to " . $ST2Name . " for the selected date and time. You can try searching for a different date and time or publish your own request for the lift. We'll let you know as soon as someone offers you a lift.</div>";
		}
	}
?>