...
<?php
	...
	$Query = "SELECT * FROM AberLift_OFFERS WHERE offer_id='$offerID'";
	...
	if (!empty($result))
	{
		...
		echo '					</div>';
		echo '					<div class="Sub-graphic-location">OFFERED BY</div>';
		echo '				</div>';
		echo '			</div>';
		...
		echo '				Please have a look at the offer carefully before accepting it. Once you accept the offer, we\'ll send your details to ';
		...
		echo '				<div id="confirmationButtonsContainer">';
		echo '					<div onclick="acceptOffer(\'' . $offerID . '\')" class="header-button confirmationButtons">Accept Offer</div>';
		echo '					<div onclick="cancelOffer()" class="center header-button confirmationButtons" style="width:65px">Cancel</div>';
		echo '				</div>';
		echo '			</div>';
	}
?>